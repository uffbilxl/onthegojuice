...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Expand line items via the Stripe API (avoids metadata char limits)
    let lineItems = [];
    try {
      ...
      // Fallback to compact metadata
      try {
        lineItems = JSON.parse(session.metadata?.items || '[]').map((i) => ({
          name: i.n,
          qty: i.q,
          price: i.p,
        }));
    ...
    const { error } = await supabaseAdmin.from('orders').insert({
      ...
      customer_phone:
        session.customer_details?.phone || meta.customer_phone || '',
    ...
    if (error) {
      ...
      // Still return 200 so Stripe does not retry — log for investigation
    }
  }
